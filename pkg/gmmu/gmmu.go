package gmmu

// Memory is the whole block of memory
// var Memory [65535]int

//RAM abstraction of the memory array so I can have helper functions that have the timing stuff
type RAM struct {
	Memory [65535]int
}

//Set sets the value of the memory address
func (ram *RAM) Set(index int, val int) {
	ram.Memory[index] = val
	/* 	if true {
	   		return ram.Memory[index]
	   	} else {
	   		return 0
	   	} */

}

//Get fetches the data from the offset [if permitted]
func (ram *RAM) Get(index uint16) int {
	return ram.Memory[index]

	/* 	if true {
	   		return ram.Memory[index]
	   	} else {
	   		return 0
	   	} */
}

//ValiditateAddress mirrors necessary stuff and warns about invalid memory writing
func (ram RAM) ValiditateAddress() {

}

// InitMMU starts the MMU
func (ram *RAM) InitMMU() {

	bootrom := []int{}
	for i := 0; i < 0xFF; i++ {
		ram.Memory[i] = bootrom[i]
	}
	print("MMU Started!\n\n")
}
